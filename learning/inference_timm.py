...
def inference_func(model, image_file: str, action_prev: Action):
    ...
    input_tensor = transform(img).unsqueeze(0).to("cuda")  # shape: [1, 3, H, W]

    start_time = time.time()

    # input the tensor (image) into the model and get the logits
    logits = model(input_tensor)
    probs = torch.nn.functional.softmax(logits, dim=1).squeeze()
    action_index = torch.argmax(probs).item()
    action_now = idx_to_action[action_index]

    end_time = time.time()

    inference_times.append(end_time - start_time)

    # Prevent cycling actions
    if (action_now == Action.ROTATE_LEFT and action_prev == Action.ROTATE_RIGHT) or \
       (action_now == Action.ROTATE_RIGHT and action_prev == Action.ROTATE_LEFT):
        action_index = torch.topk(probs, 2).indices[1].item()
        action_now = idx_to_action[action_index]
...
def parse_args():
    ...
    arg_parser.add_argument("local_model", help="Loacl path to the timm model to evaluate.")
...
def main():
    ...
    # using a local path for the model, not loading from wandb
    model_path = Path(args.local_model)  
    # match architecture to the one used in training, not using ImageNet pretrained weights, one class for each action
    model = timm.create_model('mobilenetv4_conv_small_050', pretrained=False, num_classes=3) 
    # load the model weights from the specified path, map the tensor to CPU
    checkpoint = torch.load(model_path, map_location='cpu')

    # load the checkpoint's state_dict (weights and biases) in either format
